...
describe('Bookmarks Endpoints', () => {
                ...
                return supertest(app)
                    ...
                    .expect(res => {
                        expect(res.body[0].title)
                            .to
                            .eql(expectedBookmark.title)
                        expect(res.body[0].description)
                            .to
                            .eql(expectedBookmark.description)
                    })
            })
        })
    })
        ...
        context('Given there are bookmarks in the database', () => {
            ...
            it('removes the bookmark by ID from the store', () => {
                ...
                return supertest(app)
                    ...
                    .set('Authorization', `Bearer ${process.env.API_TOKEN}`)
                    .expect(204)
                    .then(() => supertest(app).get(`/bookmarks`).set('Authorization', `Bearer ${process.env.API_TOKEN}`).expect(expectedBookmarks))
            })
        })
    })
        ...
        it('adds a new bookmark to the store', () => {
            ...
            return supertest(app)
                ...
                .expect(res => {
                    expect(res.body.title)
                        .to
                        .eql(newBookmark.title)
                    expect(res.body.url)
                        .to
                        .eql(newBookmark.url)
                    expect(res.body.description)
                        .to
                        .eql(newBookmark.description)
                    expect(res.body.rating)
                        .to
                        .eql(newBookmark.rating)
                    expect(res.body)
                        .to
                        .have
                        .property('id')
                })
                .then(res => supertest(app).get(`/bookmarks/${res.body.id}`).set('Authorization', `Bearer ${process.env.API_TOKEN}`).expect(res.body))
        })
    ...
    // describe.only starts here
    describe.only(`PATCH /bookmarks/:bookmarks_id`, () => {
      
        context(`Given no bookmakrs`, () => {
            it(`responds with 404`, () => {
                const bookmarkId = 123456
                return supertest(app)
                    .patch(`/bookmarks/${bookmarkId}`)
                    .set('Authorization', `Bearer ${process.env.API_TOKEN}`) //if this isnt included you will get a "401 unauthorized" error
                    .expect(404, {
                        error: {
